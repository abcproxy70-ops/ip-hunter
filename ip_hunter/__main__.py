...
def main() -> None:
    ...
    # Проверяем Telegram при старте
    if cfg.telegram_bot_token and cfg.telegram_admin_id:
        log_info(f"Telegram: бот настроен (admin_id={cfg.telegram_admin_id})")
        send_telegram(
            cfg.telegram_bot_token, cfg.telegram_admin_id,
            f"🚀 <b>IP Hunter запущен</b>\n"
            f"Провайдеров: {len(active)}\n"
            f"Потоков: {len(active)}",
        )
    else:
        if not cfg.telegram_bot_token:
            log_warn("Telegram: bot_token не задан — уведомления отключены")
        elif not cfg.telegram_admin_id:
            log_warn("Telegram: admin_id не задан — уведомления отключены")
